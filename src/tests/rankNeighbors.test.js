...
before(async () => {
  ...
  //   #1 nbrsalpha   score 900
  //   #2 nbrsbravo   score 500
  //   #3 nbrscharlie score 500 (same score, newer -> below bravo)
  //   #4 nbrsdelta   score 100
...
test('getRankNeighbors returns above/below with correct score gaps (middle)', async () => {
  const n = await agentService.getRankNeighbors('nbrsbravo');
  assert.equal(n.self.rank, 2);
  // above = the #1 agent we are chasing
  assert.equal(n.above.handle, 'nbrsalpha');
...
test('GET /api/agents/:id/rank/neighbors returns above + below', async () => {
  const res = await get('/api/agents/nbrsbravo/rank/neighbors');
  assert.equal(res.status, 200);
  const body = res.json();
  assert.equal(body.ranked, true);
  assert.equal(body.self.rank, 2);
  assert.equal(body.above.handle, 'nbrsalpha');
  assert.equal(body.below.handle, 'nbrscharlie');
