...
export class Kairune {
   ...
   */
  async getReceipt(spendId: string): Promise<SpendReceipt> {
    const res = await this.request<{ receipt: SpendReceipt }>('GET', `/spends/${spendId}/receipt`)
    return res.receipt
  }
   ...
   */
  async getPlatformKey(): Promise<PlatformKey> {
